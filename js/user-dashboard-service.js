var UserDashboardService = {
  ...
  last: function () {
    $.ajax({
      ...
      success: function (data) {
        ...
        $("#valid").html(
          `<div class="text-sucess text-center mt-2" id="valid"><h1>` +
            data.end_date +
            `</h1></div>`
        );
        $("#membership_name").html(
          `<div class="text-sucess text-center mt-2" id="valid"><h1>` +
            data.description +
            `</h1></div>`
        );
  ...
  get_id: function () {
    $("#end").html(
      `<div class="text-info text-center mt-2" id="end"><h1>` +
        parse_jwt(localStorage.getItem("token")).id +
        `</h1></div>`
    );
  },
